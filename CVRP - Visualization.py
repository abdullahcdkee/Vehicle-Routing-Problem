...
# Define number of nodes to be generated
numCustomers = 29
...
class EA:
    def __init__(self, nodelist, nodecoords, nodecap, size, journeys, max_cap, dist):
    ...
    # Calculate distance based on the values calculated by the map interface
    ...
    def crossover(self):
        ...
        for i in range(0,len(selected_candidates_copy)-1,2):
            ...
            child1 = self.child1reate_child(parent1, parent2)
...
# function solves the routing problem
def solve_vrp(generations, iterations, parent_size, population_size, nodelist, nodecoords, nodecap, journeys, max_cap, dist):
    ...
    for i in range(iterations):
        vrp_prob = EA(nodelist, nodecoords, nodecap, population_size, journeys, max_cap, dist)
...
# Read data file to store capacity of different nodes
...
random_route, truck_route = solve_vrp(generations, iterations, parent_size, population_size, nodelist, nodecoords, nodecap, journeys, max_cap, dist)
...
# Generate route assignments for the initial and optimal route
print("Initialized Random Route:")
assignmentsDF = vrvSolver(nodesDF, dist, time, random_route)
print()
print("Optimal Route:")
solutionDF = vrvSolver(nodesDF, dist, time, truck_route)
...
optimal_map.save('optimal_map.html')


# Create a Cesium movie showing the nodes, routes, and package deliveries for the 
# random initialized and optimal route.
vrv.createCesium(
    assignments = assignmentsDF, 
    nodes       = nodesDF, 
    startTime   = '10:00:00', 
    cesiumDir   = os.environ['CESIUMDIR'],
    problemDir  = 'combine_random_initialization_withBox')
...
vrv.createCesium(
    ...
    startTime   = '10:00:00', 
    cesiumDir   = os.environ['CESIUMDIR'],
    problemDir  = 'combine_optimal_route_withBox')
